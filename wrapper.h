...
private:
	...
	// should return false.
	virtual bool OnAccept(boost::shared_ptr<Connection> connection, const std::string & addr, boost::uint8_t channel) = 0;
...
private:
	...
	// host.
	virtual void OnAccept(const std::string &addr, boost::uint8_t channel) = 0;

	// Called when the connection has successfully connected to the remote
	// host.
	virtual void OnConnect(const std::string &addr, boost::uint8_t channel) = 0;
...
public:
	...
	void Bind(const std::string &addr, boost::uint8_t channel);

	// Starts an a/synchronous connect.
	void Connect(const std::string &addr, boost::uint8_t channel);
